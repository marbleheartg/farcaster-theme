...
const entrypoints = [
  ...
  ...new Bun.Glob("**.tsx").scanSync("src"),
  ...new Bun.Glob("**.css").scanSync("src"),
]
  .flatMap(a => path.resolve("src", a))
  .filter(dir => !dir.includes("node_modules"))
console.log(`📄 Found ${entrypoints.length} diff ${entrypoints.length === 1 ? "file" : "files"} to process\n`)
